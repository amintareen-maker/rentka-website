...
const SupplyRecipientPanel=({projection}:{projection:SupplyRecipientProjection})=><section className="mt-4 rounded-xl border-2 border-blue-200 bg-blue-50 p-4"><div><h4 className="font-black text-[#0F2B46]">Recommended suppliers</h4><p className="text-xs text-slate-600">Each vendor appears once. Owner-drivers appear as themselves.</p></div><div className="mt-3 grid gap-3">{projection.top.map((candidate,index)=><article key={candidate.id} className="rounded-lg bg-white p-3"><div className="flex flex-wrap justify-between gap-2"><div><p className="font-black">{index+1}. {candidate.displayName}</p><p className="text-sm">Type: {candidate.classification==="vendor_managed"?"Vendor first":"Owner drives"}</p></div><span className={`h-fit rounded-full px-2 py-1 text-xs font-bold ${candidate.sendReady?"bg-green-100 text-green-800":"bg-amber-100 text-amber-900"}`}>{candidate.sendReady?"Operational WhatsApp ready":"Operational WhatsApp unavailable"}</span></div><dl className="mt-2 grid gap-2 text-sm sm:grid-cols-3"><div><dt className="text-slate-500">Compatible vehicles</dt><dd className="font-bold">{candidate.eligibleVehicleCount}</dd></div><div><dt className="text-slate-500">Eligible drivers</dt><dd className="font-bold">{candidate.eligibleDriverCount}</dd></div><div><dt className="text-slate-500">Best pair score</dt><dd className="font-bold">{candidate.bestPairScore}</dd></div></dl><p className="mt-2 text-sm"><b>Best matching vehicle:</b> {candidate.bestVehicleLabel}</p><p className="mt-1 text-xs text-slate-500">{candidate.eligiblePairCount} eligible driver/vehicle combination(s), grouped as one recipient.</p></article>)}{!projection.top.length&&<p className="rounded-lg bg-white p-3 text-sm">No supply account is eligible for automated recipient selection.</p>}</div>{projection.excluded.length>0&&<details className="mt-3 rounded-lg border border-blue-200 bg-white p-3"><summary className="cursor-pointer text-sm font-bold">Driver setup or internal routing attention ({projection.excluded.length})</summary><div className="mt-2 grid gap-2 text-sm">{projection.excluded.map(item=><p key={item.supplyAccountId}><b>{item.displayName}</b> — {item.reason}</p>)}</div></details>}</section>;
