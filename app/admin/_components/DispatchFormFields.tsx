...
export function Select({ name, value, options, labels }: { name: string; value?: string; options: readonly string[]; labels?: Readonly<Record<string, string>> }) { return <select name={name} defaultValue={value} className={input}>{options.map((x) => <option key={x} value={x}>{labels?.[x] ?? x.replaceAll("_", " ")}</option>)}</select>; }
