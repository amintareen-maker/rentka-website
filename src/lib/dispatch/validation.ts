...
import { DRIVER_SUPPLY_RELATIONSHIPS, LEGACY_DRIVER_RELATIONSHIP, LEGACY_SUPPLY_CLASSIFICATION, LEGACY_VEHICLE_CONTROL, SUPPLY_CLASSIFICATIONS, VEHICLE_CONTROL_RELATIONSHIPS } from "./supply-classification.ts";
...
const selectedOrLegacy = <T extends string>(form: FormData, key: string, allowed: readonly T[], label: string, legacyValue: T): T => clean(form.get(key)) ? selected(form, key, allowed, label) : legacyValue;
