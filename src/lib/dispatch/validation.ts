...
import { CHECK_STATES, DISPATCH_PRIORITIES, DOCUMENTATION_STATES, DRIVER_STATUSES, VEHICLE_STATUSES, type CheckState, type DispatchPriority, type DocumentationState, type DriverStatus, type VehicleStatus } from "./types.ts";
...
export function parseDriverForm(form: FormData) {
  ...
  return { name: required(form, "name", "Driver name"), mobileNumber: mobile.original, mobileNumberNormalized: mobile.normalized, whatsappNumber: whatsapp.original, whatsappNumberNormalized: whatsapp.normalized, vendorId: required(form, "vendorId", "Vendor"), zoneIds: zones(form), priority: selected<DispatchPriority>(form, "priority", DISPATCH_PRIORITIES, "priority"), status: selected<DriverStatus>(form, "status", DRIVER_STATUSES, "driver status"), active: form.get("active") === "on", notes: optional(form, "notes"), documentation: { cnicVerificationState: check(form, "cnicVerificationState"), licenceState: check(form, "licenceState"), licenceExpiry: date(form, "licenceExpiry"), verifiedAt: date(form, "documentationVerifiedAt"), notes: optional(form, "documentationNotes") } };
}
