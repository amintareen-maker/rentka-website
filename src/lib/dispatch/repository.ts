...
const vehicleRecord = (doc: FirebaseFirestore.QueryDocumentSnapshot) => ({ ...record<DispatchVehicle>(doc), controlRelationship: doc.data().controlRelationship ?? LEGACY_VEHICLE_CONTROL });
const driverRecord = (doc: FirebaseFirestore.QueryDocumentSnapshot) => ({ ...record<DispatchDriver>(doc), supplyRelationship: doc.data().supplyRelationship ?? LEGACY_DRIVER_RELATIONSHIP });
...
export async function getDispatchVehicle(id: string) { const doc = await getAdminDb().collection(DISPATCH_COLLECTIONS.vehicles).doc(id).get(); return doc.exists ? ({ ...doc.data(), id: doc.id, controlRelationship: doc.data()?.controlRelationship ?? LEGACY_VEHICLE_CONTROL, createdAt: iso(doc.data()?.createdAt), updatedAt: iso(doc.data()?.updatedAt) } as DispatchVehicle) : null; }
export async function getDispatchDriver(id: string) { const doc = await getAdminDb().collection(DISPATCH_COLLECTIONS.drivers).doc(id).get(); return doc.exists ? ({ ...doc.data(), id: doc.id, supplyRelationship: doc.data()?.supplyRelationship ?? LEGACY_DRIVER_RELATIONSHIP, createdAt: iso(doc.data()?.createdAt), updatedAt: iso(doc.data()?.updatedAt) } as DispatchDriver) : null; }
