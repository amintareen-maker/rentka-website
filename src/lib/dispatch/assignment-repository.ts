...
export async function getAssignmentPanel(bookingOperationalId:string){
 ...
 const availableOffers=offers.filter(offer=>offer.responseStatus==="available"||offer.responseStatus==="accepted");return{booking,matches,offers,availableOffers,vendors,drivers,vehicles};
}
...
export async function assignOperationalBooking(bookingOperationalId:string,selection:AssignmentSelection,input?:{reason?:string}){
 ...
 return db.runTransaction(async tx=>{
  const[bookingSnap,driverSnap,vehicleSnap,offerSnap,reservationsSnap,allOffersSnap,broadcastControlSnap]=await Promise.all([tx.get(bookingRef),tx.get(driverRef),tx.get(vehicleRef),tx.get(offerRef),tx.get(db.collection(DISPATCH_COLLECTIONS.reservations).where("active","==",true)),tx.get(bookingRef.collection(OFFERS)),tx.get(bookingRef.collection("dispatchBroadcastControl").doc("current"))]);
  ...
  if(booking.assignment?.status==="assigned"){if(booking.assignment.assignedDriverId===selection.driverId&&booking.assignment.assignedVehicleId===selection.vehicleId)return{assignment:booking.assignment,duplicate:true};const tripSnap=await tx.get(bookingRef.collection("tripOperations").doc(booking.assignment.id));assertTripAllowsReassignment(tripSnap.exists?tripSnap.data()!.currentStatus:undefined);if(!input?.reason||input.reason.trim().length<8)throw new Error("A specific reassignment reason of at least 8 characters is required.");}
  ...
  const validated=validateAssignmentSelection({booking,matches,offers:[offer],vendors:[vendor],drivers:[driver],vehicles:[vehicle],selection,activeBroadcastId:String(broadcastControlSnap.data()?.activeBroadcastId??"")}),window=matchingWindow(booking),now=FieldValue.serverTimestamp(),snapshots=assignmentSnapshots(driver,vehicle),previous=booking.assignment?.status==="assigned"?booking.assignment:undefined;
  const agreedPayout=validated.offer.agreedPayoutMinor??validated.offer.currentOfferedPayoutMinor??booking.internalFinancials.vendorPayoutMinor!,assignment={id:assignmentRef.id,status:"assigned" as const,bookingOperationalId:booking.id,bookingId:booking.bookingId,dispatchVendorId:vendor.id,vendorName:vendor.name,assignedDriverId:driver.id,driverSnapshot:snapshots.driverSnapshot,assignedVehicleId:vehicle.id,vehicleSnapshot:snapshots.vehicleSnapshot,approvedVendorPayoutMinor:agreedPayout,offerId:validated.offer.id,offerCandidateId:validated.offer.candidateId,assignedAt:now,assignedBy:sharedActor,window,...(previous?{previousAssignmentId:previous.id,reason:input!.reason!.trim()}:{})};
  ...
  for(const item of allOffersSnap.docs){tx.set(item.ref,{portalStatus:"closed",closeReason:item.id===offer.id?"selected_for_assignment":"trip_no_longer_available",updatedAt:now},{merge:true})}
  tx.create(bookingRef.collection("events").doc(),auditEvent("driver_offers_closed" as never,now,{reason:"booking_assigned",selectedOfferId:offer.id,closedOfferIds:allOffersSnap.docs.map(item=>item.id)}));
  tx.create(bookingRef.collection("events").doc(),auditEvent(previous?"booking_reassigned":"booking_assigned",now,{assignmentId:assignmentRef.id,driverId:driver.id,vehicleId:vehicle.id,vendorId:vendor.id,approvedVendorPayoutMinor:agreedPayout,offerId:offer.id,candidateId:offer.candidateId,...(previous?{previousAssignmentId:previous.id,reason:input!.reason!.trim()}: {})}));
