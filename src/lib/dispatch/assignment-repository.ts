...
export async function assignOperationalBooking(bookingOperationalId:string,selection:AssignmentSelection,input?:{reason?:string}){
 ...
 return db.runTransaction(async tx=>{
  const[bookingSnap,driverSnap,vehicleSnap,offerSnap,reservationsSnap,allOffersSnap]=await Promise.all([tx.get(bookingRef),tx.get(driverRef),tx.get(vehicleRef),tx.get(offerRef),tx.get(db.collection(DISPATCH_COLLECTIONS.reservations).where("active","==",true)),tx.get(bookingRef.collection(OFFERS))]);
