...
export default function LahoreBookingClient({ inventory, context, variant = "private-test" }: Props) {
  ...
  const grouped = useMemo(() => Object.values(inventory.reduce<Record<string, LahoreBookingInventory[]>>((result, item) => {
    (result[item.modelKey] ??= []).push(item); return result;
  }, {})), [inventory]);
  ...
  const inventoryGridClass = grouped.length === 1
  ...
  return <GooglePlacesProvider><>
    <div className={inventoryGridClass}>{grouped.map((options) => { const first = options[0]; const minimum = Math.min(...options.map((item) => item.pricing.withDriver.withinCity.daily ?? Infinity)); const hasWithinCity = options.some((item) => item.pricing.withDriver.withinCity.daily !== undefined); const hasOutstation = options.some((item) => item.pricing.withDriver.outsideCity.daily !== undefined); return <button key={first.modelKey} type="button" aria-haspopup="dialog" onClick={() => setModelOptions(options)} className="group rounded-3xl border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-[#5BAE4A] hover:shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#5BAE4A] sm:p-6"><div className="relative h-56 sm:h-64"><Image src={first.imageURL} alt={`${first.modelName} available with driver in Lahore`} fill unoptimized sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 640px" className="object-contain transition duration-300 group-hover:scale-[1.03]"/></div><div className="mt-5 flex flex-wrap gap-2 text-xs font-bold text-slate-700"><span className="rounded-full bg-green-50 px-3 py-1.5 text-green-800">Driver included</span>{hasWithinCity && <span className="rounded-full bg-slate-100 px-3 py-1.5">Within Lahore</span>}{hasOutstation && <span className="rounded-full bg-slate-100 px-3 py-1.5">Outstation</span>}</div><h3 className="mt-4 text-2xl font-black text-[#0F2B46]">{first.modelName}</h3><p className="mt-3 text-lg font-black text-[#5BAE4A]">From PKR {minimum.toLocaleString("en-PK")} <span className="text-sm font-semibold text-slate-500">/ day</span></p><span className="mt-5 inline-flex min-h-11 items-center rounded-xl bg-[#0F2B46] px-5 py-3 text-sm font-bold text-white">View Packages →</span></button>; })}</div>
    {modelOptions && !selected && <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-0 sm:items-center sm:p-4"><div role="dialog" aria-modal="true" aria-labelledby="lahore-options-title" className="max-h-[90dvh] w-full max-w-3xl overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl sm:max-h-[85vh] sm:rounded-2xl sm:p-6"><button type="button" aria-label="Close vehicle options" onClick={() => setModelOptions(undefined)} className="float-right rounded-lg p-2 font-bold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#5BAE4A]">✕</button><h2 id="lahore-options-title" className="text-2xl font-black text-[#0F2B46]">{modelOptions[0].modelName}</h2><p className="mt-2 text-sm text-slate-600">Compare the available package prices, then choose the option that suits your trip.</p><div className="mt-5 space-y-3">{modelOptions.map((option, index) => <button key={option.inventoryId} type="button" onClick={() => setSelected(option)} className="w-full rounded-xl border p-4 text-left hover:border-[#5BAE4A] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#5BAE4A]"><p className="font-bold">{prelaunch ? `Car option ${index + 1}` : option.vendorName}</p><div className="mt-2 grid gap-2 text-sm sm:grid-cols-2"><p>Within Lahore: PKR {option.pricing.withDriver.withinCity.daily?.toLocaleString("en-PK")}</p><p>Outstation: PKR {option.pricing.withDriver.outsideCity.daily?.toLocaleString("en-PK")}</p></div></button>)}</div></div></div>}
    {selected && <div className="fixed inset-0 z-50 overflow-y-auto bg-black/50 p-0 sm:p-4"><div role="dialog" aria-modal="true" aria-labelledby="lahore-booking-title" className="mx-auto mt-8 min-h-[calc(100dvh-2rem)] max-w-2xl rounded-t-3xl bg-white p-5 shadow-2xl sm:my-6 sm:min-h-0 sm:rounded-2xl sm:p-6"><button type="button" aria-label="Close booking form" onClick={() => { setDateOpen(false); setTimeOpen(false); setSelected(undefined); setCreated(undefined); }} className="float-right rounded-lg p-2 font-bold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#5BAE4A]">✕</button><p className="text-xs font-bold uppercase tracking-wider text-[#5BAE4A]">{context.cityLabel} · With Driver</p><h2 id="lahore-booking-title" className="mt-2 text-2xl font-black text-[#0F2B46]">{selected.modelName}</h2>{!prelaunch && <p className="mt-1 text-sm text-slate-600">{selected.vendorName}</p>}
